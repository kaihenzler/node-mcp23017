...
var MCP23017 = (function() {
  ...
  MCP23017.prototype.FALLING = 0;
  MCP23017.prototype.RISING = 1;
  MCP23017.prototype.CHANGE = 2;
  MCP23017.prototype.ACTIVE_LOW  = 0x000;
  MCP23017.prototype.ACTIVE_HIGH = 0x010;
  MCP23017.prototype.OPEN_DRAIN  = 0x100;
  ...
  MCP23017.prototype.reset = function () {
    ...
    for (var i = 0; i < 2; i++) {
      this.inten[i] = 0x00;
      this.send(INTEN_ADDR[i], this.inten[i]);
      this.defval[i] = 0x00;
      this.send(DEFVAL_ADDR[i], this.defval[i]);
      this.intcon[i] = 0x00;
      this.send(INTCON_ADDR[i], this.intcon[i]);
    }
    this.send(IOCON_ADDR, 0x00)
    this._initGpioA();
    this._initGpioB();
  };
  ...
  */
  MCP23017.prototype.interruptMode = function (mirror, mode) {
    if (mode !== this.ACTIVE_LOW && mode !== this.activeHigh && mode !== this.OPEN_DRAIN) {
  ...
  /*
    configures an pin interrupt
  */
  MCP23017.prototype.attatchInterrupt = function (pin, mode) {
    if (mode !== this.CHANGE && mode !== this.RISING && mode !== this.FALLING) {
      console.error('invalid value', mode);
      return;
    }
    if (isNaN(pin)) {
    ...
    this._setInterrupt(pin >= 8 ? pin - 8 : pin, dir, mode, pin >= 8 ? 1 : 0);
  };
  ...
  */
  MCP23017.prototype.getInterruptCapture = function (port, callback) {
    ...
    //read one byte from the right register (A or B)
    this._read(IOCAP_ADDR[port], 1, function (err, registerValue) {
      if (err) {
        console.error(err);
        callback(_port, err, null);
      } else {
